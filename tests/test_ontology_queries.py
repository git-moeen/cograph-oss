...
GRAPH = "https://omnix.dev/graphs/test"


def test_type_uri():
    assert type_uri("Place") == "https://omnix.dev/types/Place"


def test_attr_uri():
    assert attr_uri("Place", "name") == "https://omnix.dev/types/Place/attrs/name"


def test_insert_type_basic():
    sparql = insert_type(GRAPH, "Place")
    assert "INSERT DATA" in sparql
    assert "GRAPH <https://omnix.dev/graphs/test>" in sparql
...
def test_insert_attribute():
    ...
    assert "INSERT DATA" in sparql
    assert "omnix.dev/types/Place/attrs/name" in sparql
...
def test_insert_subtype():
    ...
    assert "subClassOf" in sparql
    assert "omnix.dev/types/Park" in sparql
    assert "omnix.dev/types/Place" in sparql
...
def test_list_types_query():
    ...
    assert "Class" in sparql
    assert "FROM <https://omnix.dev/graphs/test>" in sparql
